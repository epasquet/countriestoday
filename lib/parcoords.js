...
var blue_to_red = d3.scale.quantize()
                    .domain([0, 1]) // update Sidoine
                    .range(["red", "orange", "pink", "steelblue", "green"]) //update Sidoine
...
function parallelCoordinates(data) {
                                    ...
                                    // render parallel coordinates
                                       ...
                                       d3.select(".legend")
                                          .append("text")
                                          .text("country to invest")
                                          .attr("transform", "translate(0,-25)")

                                       d3.select(".legend")
                                          .append("text")
                                          .text("(based on economics datas)")
...
//START MAP
                                        ...
                                        function update_map_with_counties(counties) {
                                                ...
                                                if (counties.length != 263){
                                                  ...
                                                  /*
                                                  for (var i = 0; i < counties.length; i++){
                                                      var coun = counties[i].toLowerCase();
                                                      d3.selectAll("." + coun).style("fill", "red")
                                                  }
                                                  */
                                                  /*
                                                  var county_objects = d3.selectAll(".country")
                                                  var county_objects_selected = county_objects.filter(
                                                              function(d, i) {
                                                                                county_index = d.properties.name.toLowerCase()
                                                                                return counties.indexOf(county_index) != -1;
                                                              });

                                                  var county_objects_not_selected = county_objects.filter(
                                                              function(d, i) {
                                                                                county_index = d.properties.COUNTY.toLowerCase()
                                                                                return counties.indexOf(county_index) == -1;
                                                              });
                                                  //console.log(county_objects_selected)
                                                  county_objects_selected.style('fill', "#111111");
